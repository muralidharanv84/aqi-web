...
export function ComparisonTooltip({active, label, names, series, metric, resolution}: TooltipProps<number, string> & {
  names: string[];
  ...
  return (
    <div className="max-w-xs space-y-3 rounded-xl border border-slate-200 bg-white p-3 text-xs shadow-lg">
      ...
      {names.map((name, index) => {
        const point = findClosestReading(series[index], label / 1000);
        return (
          <div key={name}>
            <div className="break-all font-medium" style={{color: COMPARISON_COLORS[index]}}>{name}</div>
            <div className="mt-1 font-semibold text-slate-900">
              {point ? formatMetricValue(metric.key, point.value) : "No readings in this range"}
            </div>
            {point && point.ts !== label / 1000 ? (
              <div className="mt-1 text-slate-500">
                {resolution === "raw" ? "Closest reading" : "Closest period"} · {formatChartPeriod(point.ts * 1000, resolution)}
              </div>
            ) : null}
